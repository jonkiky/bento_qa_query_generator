...
MATCH (samp)<-[:file_of_sample]-(f)-[:file_of_laboratory_procedure]->(lp) WITH
    distinct lp,
    samp, ss, s, p ,d
    RETURN
"""
...
sample_tab_return = {
  ...
  "Diagnosis":"ss.disease_subtype AS `Diagnosis`",
  "Tissue Type":"samp.tissue_type AS `Tissue Type`",
  "Tissue Composition":"samp.composition AS `Tissue Composition`",
  "Sample Anatomic Site":"samp.sample_anatomic_site AS `Sample Anatomic Site`",
  "Sample Procurement Method":"samp.method_of_sample_procurement AS `Sample Procurement Method`",
  "platform":"lp.test_name AS `platform`",
  "Tumor Size":"d.tumor_size_group as `Tumor Size`",
}
